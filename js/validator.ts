...
export function isValidPieceString(figure: string, white?: boolean): boolean {
...
export function isValidFen(
	fen: string,
	variant: number = VARIANTS.UNKNOWN,
): boolean {
...
function isValidBoard(variant: number, fields: string[]): boolean {
	if (variant !== VARIANTS.DEFAULT)
		throw new ValidationError("Chess Variant not supported");

	//* kings
	// 1 king of each color
	const kMatches = fields[0].match(/k/g);
	const KMatches = fields[0].match(/K/g);
	if (!kMatches || kMatches.length !== 1 || !KMatches || KMatches.length !== 1)
  ...
  */
	...
	const pMatches = fields[0].match(/p/g);
	const PMatches = fields[0].match(/P/g);
	if (!((pMatches?.length ?? 0) <= 8 && (PMatches?.length ?? 0) <= 8))
